...
\begin{theorem}
    If $\T$ has $\Sigma$-types (with both computation/$\beta$ and uniqueness/$\eta$ rules \cite{nlab}) and a unit type, then the closure under isomorphism of the class of maps of the form $\pi_\Gamma : \Gamma. A \to \Gamma$ is a clan (and indeed, is the same class as in \Cref{thm:syn-clan}).
\end{theorem}
\begin{proof}
...
\begin{exercise}
    Construct a category with families from an arbitrary display structure, and show that when you apply this to the display structure of \Cref{thm:syn-display}, you obtain the same category with families as above. 
\end{exercise}
